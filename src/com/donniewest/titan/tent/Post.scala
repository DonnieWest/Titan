...
object Post {
  ...
  def send_post(text: String) {
    ...
    HttpRequest.post(Endpoints.getNew_post).contentType(contentType).authorization(Hawk_Headers.build_headers(json,"POST",Endpoints.getNew_post, "application/vnd.tent.post.v0+json")).send(json).body
  ...
  def retrieve_feed() {
    ...
    val json_post_feed = parse(HttpRequest.get(Endpoints.getPost_feed).accept("application/vnd.tent.posts-feed.v0+json").authorization(Hawk_Headers.build_headers("","GET",Endpoints.getPost_feed, "application/vnd.tent.post.v0+json", false)).body)
